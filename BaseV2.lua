--[[
    ...
    Note:
    - This has been ported to work for the old Drawing system and therefore is missing outlines
]]
...
local DefaultFont = Drawing.Fonts.Monospace
local Vertices = {
	Vector3.new(-1, -1, -1),
	Vector3.new(-1,  1, -1),
	Vector3.new(-1,  1,  1),
...
-- // Special functions
local function worldtoscreen(points)
    -- // Vars
    local CurrentCamera = Workspace.CurrentCamera
    local ScreenPoints = {}

    -- // Loop through each point and convert each one
    for i, v in pairs(points) do
        ScreenPoints[i] = Utilities.ConvertV3toV2(CurrentCamera:WorldToViewportPoint(v))
    end
...
do
    ...
    function Utilities.ConvertV3toV2(Vector)
        ...
        elseif (VectorType == "Vector2") then
            return Vector
        end
